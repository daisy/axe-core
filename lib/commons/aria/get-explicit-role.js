...
function getExplicitRole(vNode, { fallback, abstracts, dpub } = {}) {
  ...
  const firstValidRole = roleList.find(role => {
    // DPUB support enabled by default in this DAISY/EPUB fork!
    // dpub === undefined (typeof dpub === "undefined") means true!
    const disableDPUB = dpub === false;
    if (disableDPUB && role.substr(0, 4) === 'doc-') {
      return false;
    }
